''' get_ADInfo: utilities to crawl FAAH files

        The primary routine is:

    mglTop_visit_AD(outdir)

    which produces summary and inter files for each batch in
    outdir/summ and outdir/inter subdirectories, resp. that look like:

    AD_summ_22640:
    Expt,Recept,Ligand,E,Eff,Nvdw,Ninter
    xEyeSiteXtl5NI,xEyeSiteXtl5NI,ZINC00784756,-7.54,-0.279,46,3

    AD_inter_22640:
    Expt,Recept,Lig,IType,Liname,Rchain,Raa,Ratom
    xEyeSiteXtl5NI,xEyeSiteXtl5NI,ZINC00784756,hba,O,A,GLY48,N

    It crawls all batches found under AD_topDir
    AD_topDir = '/mgl/storage/wcg_permanentARCHIVE/processed/'
    untarring and uncompressing all tarballs as required

    another useful ad hoc routine is:

    getMultPDBQT(plus2getList,outdir)

    which takes a list of (batch,expt,ligand) specs and
    extracts the request PDBQT+ file into outdir
